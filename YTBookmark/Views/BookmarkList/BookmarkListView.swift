...
struct BookmarkListView: View {
    ...
    var body: some View {
        ...
        .navigationDestination(for: NavigationDestination.self) { destination in
            switch destination {
            case .folders:
                // Phase 2C: replaced with FolderListView()
                Text("Folders — coming in Phase 2C")
            case .folderDetail(let folder):
                // Phase 2C: replaced with FolderDetailView(folder: folder)
                Text("Folder: \(folder.name) — coming in Phase 2C")
            }
        }
